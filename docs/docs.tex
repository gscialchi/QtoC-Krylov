...
This document contains definitions and analytic derivations that are used in the code base but remain otherwise unexplained in the manuscript \cite{scialchi2026quantumtoclassicalcorrespondencekrylovcomplexity},
...
i.e., let $\beta^{(l)}$ be the $l\times l$ matrix with elements $(\beta^{(l)})_{n t} = \bnt{n}{t}$ and $\alpha^{(l)}$ the $l\times l$ matrix with elements $(\alpha^{(l)})_{tm} = \atn{t}{m}$,
then $\beta^{(l)} \alpha^{(l)} = \mathbb{I}_{l\times l}$.
