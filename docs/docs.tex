...
$$
\rho_{nm} = \iint drd\vphi\, f(r\cos(\vphi), r\sin(\vphi))
...
$$
\boxed{
\smel{\kr_n}{\PF}{\kr_m} = \sum_{t=0}^n \atn{t}{n} \bnt{n}{t+1}
}.
$$
Now we need $\atn{t}{n}$.
Using the orthogonality of the Krylov states $\sbraket{k_n}{k_m} = \delta_{nm}$ and Eqs.\eqref{eq:kn_rhot} and \eqref{eq:rhot_kn} it is easy to see that
$$
\sum_{t=0}^m \bnt{n}{t} \atn{t}{m} = \delta_{nm},
$$
which means that $\beta$ and $\alpha$ are recursively inverse matrices,
i.e., let $\beta^{(n)}$ be the $n\times n$ matrix with elements $(\beta^{(n)})_{n t} = \bnt{n}{t}$ and $\alpha^{(n)}$ the $n\times n$ matrix with elements $(\alpha^{(n)})_{tm} = \atn{t}{m}$,
then $\beta^{(n)} \alpha^{(n)} = \mathbb{I}_{n\times n}$.
