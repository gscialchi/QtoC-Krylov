...
cks_qu = np.zeros((len(hs), n_steps))
for i, h in enumerate(hs):
    nsig = 3 # n sigma padding for integ lims & cutoffs
    ...
    wave_qu = krylov_wavefunction_operator(rho_evo, rho_kry, hbar=h)
    cks_qu[i, :] = krylov_complexity(wave_qu)

### compare
figname = f'manuscript/HO_complexity_correspondence'
plot_complexity_correspondence(ck_cl, cks_qu, hs,
                               #  save=True,
